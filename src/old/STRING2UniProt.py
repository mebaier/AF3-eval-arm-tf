...
def translate_and_save(string_ids):
    """Translate STRING IDs to UniProt IDs and save the results."""
    job_id = submit_id_mapping(from_db="STRING", to_db="UniProtKB", ids=list(string_ids))
...
def translate_and_save_in_batches(string_ids, batch_size=1000):
    ...
    for i in range(0, len(string_ids), batch_size):
        ...
        job_id = submit_id_mapping(from_db="STRING", to_db="UniProtKB", ids=batch)
        if check_id_mapping_results_ready(job_id):
            link = get_id_mapping_results_link(job_id)
            results = get_id_mapping_results_search(link)
            with open(OUTPUT_FILE, "a") as f:
...
if __name__ == "__main__":
    ...
    if not new_ids:
        ...
        translate_and_save_in_batches(new_ids)
